...
card.innerHTML =   <div class="artisan-header">   <div class="artisan-avatar">   <img src="${a.photo}" alt="${a.name}" class="artisan-photo">   <div class="verified-badge" title="Artisan vérifié">   <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">   <path d="M20 6L9 17l-5-5"></path>   </svg>   </div>   </div>   <div class="artisan-info">   <p class="artisan-name">${a.name}</p>   <p class="artisan-category">${a.category}</p>   <div class="artisan-rating">   <div class="stars">${renderStars(a.rating)}</div>   <span class="rating-text">${a.rating.toFixed(1)} (${a.reviews} avis)</span>   </div>   </div>   </div>   <div class="artisan-badges">   <span class="badge ${a.availability==='Disponible'?'badge-available':'badge-busy'}">${a.availability}</span>   <span class="badge">Distance: ${a.distance} km</span>   <span class="badge">Prix: ${a.price}</span>   </div>  ;
...
container.innerHTML =   <div class="profile-hero">   <div class="profile-avatar-large">   <img src="${artisan.photo}" alt="${artisan.name}" class="profile-photo-large">   <div class="verified-badge-large" title="Artisan vérifié">   <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">   <path d="M20 6L9 17l-5-5"></path>   </svg>   </div>   </div>   <h2 class="profile-name-large">${artisan.name}</h2>   <p class="profile-category-large">${artisan.category}</p>   <div class="profile-rating-large">   <div class="stars">${renderStars(artisan.rating)}</div>   <span>${artisan.rating.toFixed(1)} (${artisan.reviews} avis)</span>   </div>   </div>   <div class="info-section">   <div class="info-card">Distance: ${artisan.distance} km</div>   <div class="info-card">Prix: ${artisan.price}</div>   <div class="info-card">Disponibilité: ${artisan.availability}</div>   </div>   <div class="artisan-actions">   <button class="btn-whatsapp">WhatsApp</button>   <button class="btn-call">Appeler</button>   </div>  ;
}
